...
SELECT e.id, e.first_name AS "First Name", e.last_name AS "Last Name", r.title, r.salary
FROM employees e
...
SELECT CONCAT(e.first_name," " ,e.last_name) AS full_name, r.title, e.manager_id
FROM employees e
...
VALUES('Tim', 'Jone', (SELECT id
        FROM roles
        WHERE title = 'Sales Rep' ),
        (SELECT id
        FROM (SELECT id
            FROM employees
            WHERE CONCAT(first_name," ",last_name) = "Michael Scott" )AS tmptable));
